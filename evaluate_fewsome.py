...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, vectors, label):
        ...
        for i in vectors:
            euclidean_distance += F.pairwise_distance(output1, i) / torch.sqrt(
                torch.Tensor([output1.size()[1]])
            ).to(self.device)
        ...
        loss_contrastive = ((1 - label) * torch.pow(euclidean_distance, 2) * 0.5) + (
            (label)
            * torch.pow(
                torch.max(torch.Tensor([torch.tensor(0), marg - euclidean_distance])), 2
            )
            * 0.5
        )
...
def evaluate(
    ...
    loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=1, shuffle=True, num_workers=1, drop_last=False
    )
    outs = (
        {}
    )  # a dictionary where the key is the reference image and the values is a list of the distances between the reference image and all images in the test set
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            )  # get feature vector (representation) for test image
            ...
            for j in range(0, num_ref_eval):
                euclidean_distance = (
                    F.pairwise_distance(out, ref_images["images{}".format(j)])
                    / torch.sqrt(torch.Tensor([out.size()[1]])).to(device)
                ) + (
                    alpha
                    * (
                        F.pairwise_distance(out, anchor)
                        / torch.sqrt(torch.Tensor([out.size()[1]])).to(device)
                    )
                )
    ...
    # save df, auc, f1, spec, recall, acc to current directory
    if not os.path.exists("./outputs/class_" + str(normal_class)):
        os.makedirs("./outputs/class_" + str(normal_class))
    df.to_csv(
        "./outputs/class_"
        + str(normal_class)
        + "/df_"
    ...
    # save auc, f1, acc, spec, recall to each column
    new_data = {"auc": auc, "f1": f1, "acc": acc, "spec": spec, "recall": recall}

    csv_path = "./outputs/class_" + str(normal_class) + ".csv"
...
def parse_arguments():
    ...
    parser.add_argument(
        "--model_type",
        choices=["CIFAR_VGG3", "CIFAR_VGG4", "MNIST_VGG3", "RESNET"],
        required=True,
    )
    ...
    parser.add_argument(
        "-i",
        "--index",
        help="string with indices separated with comma and whitespace",
        type=str,
        default=[],
        required=False,
    )
...
if __name__ == "__main__":
    ...
    val_auc, val_loss, val_auc_min, f1, acc, df, ref_vecs, inf_times, total_times = (
        evaluate(
            anchor,
            args.seed,
            base_ind,
    ...
    if not os.path.exists(string):
        os.makedirs(string)
    pd.DataFrame([params], columns=cols).to_csv(
        "./outputs/class_" + str(args.normal_class) + "/" + args.model_name
    )
